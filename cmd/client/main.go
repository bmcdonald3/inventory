...
import (
	...
	"github.com/user/inventory-api/pkg/client"
)
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&serverURL, "server", "http://localhost:8081", "inventory_api server URL")
...
Examples:
  # Create from stdin
  echo '{"name": "example"}' | client device create

  # Create with --spec flag
  client device create --spec '{"name": "example"}'
...
Examples:
  # Update from stdin
  echo '{"name": "example"}' | client device update <uid>

  # Update with --spec flag
  client device update <uid> --spec '{"name": "example"}'
